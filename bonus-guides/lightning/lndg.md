...
{% hint style="info" %}
If you obtain "**command not found**" outputs, you need to follow the PostgreSQL bonus guide installation process before continuing with this guide
{% endhint %}
...
[venv](https://docs.python.org/3/library/venv.html) is a module that supports creating lightweight "virtual environments". We will use _**venv**_ to islolate the process of building and running the tools associated with LNDg so that we can avoid conflicts with our system's Python installation and other Python projects.
...
```
lrwxrwxrwx 1 lndg lndg 9 Nov 10 01:03 .lnd -> /data/lnd
```
...
* Initialize necessary settings for your Django site. A _**FIRST TIME LOGIN PASSWORD**_ will be generated - save it somewhere safe.
...
By default, LNDg stores the LN node routing statistics and settings in a SQLite database. We'll update the `DATABASES` section of the initialized `settings.py` file to use our newly created `lndg` database from the [Create PostgreSQL database](broken-reference) section.
...
In the security section, we set up Nginx as a reverse proxy. Now we can add the LNDg configuration.
...
* The default login is `lndg-admin` and the first time password was generated by the `initialize.py` script in the initialization step. If you forgot or didn't write it down, you can find the password at `/home/lndg/lndg/data/password.txt` (you should delete this file for security reasons).
* _You can change the initial password after logging into in the GUI._
...
LNDg offers the possibility to create links to blockchain and lightning explorers on node aliases and transaction IDs. By default, LNDg uses public website _mempool.space_ for both explorers.
...
* Find the `NET URL` option and paste the following value if you use BTC RPC Explorer:
